...
- (void)dealloc {
    ...
    [self unregisterFromCustomNotifications];
#if !__has_feature(objc_arc)
	[color release];
	[indicator release];
	[label release];
...
#pragma mark - custom notifications and handlers

-(void)registerCustomNotification
{
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(hideProgressBar:) name:@"MBHUDWillHideProgressBar" object:nil];
}

-(void)unregisterFromCustomNotifications
{
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"MBHUDWillHideProgressBar" object:nil];
}
